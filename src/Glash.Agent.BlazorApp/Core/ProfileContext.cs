...
namespace Glash.Agent.BlazorApp.Core
{
    public class ProfileContext : IDisposable
    {
        ...
        {
            Status = $"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}: {Global.Instance.TextManager.GetText(BlazorApp.Model.Profile.Texts.Disconnected)}";
            var currentCts = cts;
            if (currentCts == null)
                return;
        ...
        private async Task beginConnect(CancellationToken token)
        {
            try
            {
                ...
                Status = $"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}: {Global.Instance.TextManager.GetText(BlazorApp.Model.Profile.Texts.Connected)}";
            }
            catch (Exception ex)
            {
